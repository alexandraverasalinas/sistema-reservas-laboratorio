...
export class Calendario implements OnInit {
  ...
  consultarDisponibilidad(): void {
    ...
    fetch(url)
      ...
      .catch(error => {
        ...
        this.cargando = false;
        this.mensajeError = 'No se pudo consultar la disponibilidad.';
        this.cdr.detectChanges();
      });
  }
  ...
  esMiReserva(bloque: Disponibilidad): boolean {
    if (!this.usuario || !bloque.alumnos) {
      return false;
    }
    const nombreCompleto = `${this.usuario.nombres} ${this.usuario.apellidos}`;
    return bloque.alumnos.includes(nombreCompleto);
  }
